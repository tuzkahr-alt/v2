...
const INITIAL_DATA = {
    ...
    purchases: [],
    quotas: [],
    letras: [], // Letras de cambio generadas
    creditMovements: [],
    cart: [],
    currentClient: null,
    workers: [
...
class Database {
    ...
    addWorker(worker) {
        const newWorker = { 
            id: generateId(), 
            instanceId: 'DB_' + generateId().toUpperCase(), // ID for local isolation
            ...worker 
        };
    ...
    deleteWorker(id) {
        if (id === 'admin') return false; // Cannot delete primary admin
    ...
    updateProduct(id, updatedProd) {
        ...
        if (index !== -1) {
            this.data.products[index] = { ...this.data.products[index], ...updatedProd };
            this.save();
            return true;
        }
        return false;
    ...
    addPerson(person) {
        ...
        const existing = this.data.people.find(p => p.rut.replace(/\./g,'') === person.rut.replace(/\./g,''));
        if (existing) {
            console.warn("RUT duplicado detectado:", person.rut);
            return existing; // Retornamos el existente para seleccionarlo
        }
    ...
    registerSale(paymentMethod, isPresale, creditParams = null) {
        if (this.data.cart.length === 0) return { success: false, reason: "Carrito Vacío" };

        // 1. Validar Stock si está activada la restricción
        if (this.data.posConfig.noSalesWithoutStock && !isPresale) {
            for (const item of this.data.cart) {
                const prod = this.data.products.find(p => p.id === item.id);
                if (prod && prod.stock < item.qty) {
                    return { success: false, reason: `Stock insuficiente para: ${prod.name}` };
                }
            }
        }

        // 2. Validar que la caja esté abierta (Simulación de regla de negocio)
        if (!this.data.isCashDrawerOpen && !isPresale) {
            return { success: false, reason: "Debe realizar la APERTURA DE CAJA antes de vender." };
        }
        ...
        if (paymentMethod === 'credit' && this.data.currentClient) {
            ...
            if (creditParams) {
                ...
                for (let i = 1; i <= creditParams.installments; i++) {
                    ...
                    // Si es el esquema MrPOS, guardamos explícitamente en 'letras'
                    if (creditParams.installments === 3) {
                        if (!this.data.letras) this.data.letras = [];
                        this.data.letras.push({...quota});
                    }
                }
            }
        }
    ...
    openCashDrawer(initialAmount) {
        this.data.isCashDrawerOpen = true;
        this.data.cashDrawerOpenTime = new Date().toISOString();
        this.data.cashDrawerInitialAmount = initialAmount;
        if (!this.data.cashLog) this.data.cashLog = [];
        this.data.cashLog.push({
            type: 'Apertura',
    ...
    closeCashDrawer(closingAmount) {
        this.data.isCashDrawerOpen = false;
        this.data.cashLog.push({
            type: 'Cierre',
